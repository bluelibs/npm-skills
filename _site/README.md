...
For Deno installs, dependency lifecycle scripts are also blocked by default. Use a local `node_modules` mode plus `--allow-scripts=npm:npm-skills` when installing if you want `npmSkills.auto` to run automatically:

```bash
deno install --node-modules-dir=auto --allow-scripts=npm:npm-skills
```
...
For Deno, the package resolves through the normal ESM export conditions. If you need a direct file-path import inside a Node-style project, the CommonJS build is still available.
